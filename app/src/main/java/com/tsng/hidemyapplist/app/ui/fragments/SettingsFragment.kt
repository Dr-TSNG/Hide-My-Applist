...
class SettingsFragment : PreferenceFragmentCompat() {
    private val backupImportSAFLauncher = registerForActivityResult(ActivityResultContracts.GetContent()) { uri ->
        ...
        val tmpDir = "${MyApplication.appContext.cacheDir.path}/backup"
        try {
            val tmpFile = "$tmpDir/backup.tar.gz"
            File(tmpDir).mkdirs()
            MyApplication.appContext.contentResolver.openInputStream(uri).use { fis ->
            ...
            File("${MyApplication.appContext.dataDir.path}/shared_prefs").deleteRecursively()
            Runtime.getRuntime().exec("mv $tmpDir/shared_prefs ${MyApplication.appContext.dataDir.path}").waitFor()
            Thread.sleep(50)
            // TODO: Stub, 设置页的没刷新
    ...
    private val backupExportSAFLauncher = registerForActivityResult(ActivityResultContracts.CreateDocument()) { uri ->
        if (uri == null) return@registerForActivityResult
        MyApplication.appContext.cacheDir.mkdir()
        val tmpFile = "${MyApplication.appContext.cacheDir.path}/backup.tar.gz"
        File(tmpFile).delete()
        File("${MyApplication.appContext.dataDir.path}/version").writeText(BuildConfig.VERSION_CODE.toString())
        Runtime.getRuntime().exec("tar -czf $tmpFile -C ${MyApplication.appContext.dataDir.path} version shared_prefs").waitFor()
        Thread.sleep(50)
        File(tmpFile).inputStream().use { fis ->
            MyApplication.appContext.contentResolver.openOutputStream(uri).use { fos ->
                if (fos == null) makeToast(R.string.settings_export_failed)
                else fis.copyTo(fos)
            }
        }
        File("${MyApplication.appContext.dataDir.path}/version").delete()
        File(tmpFile).delete()
    ...
    private fun module() {
        ...
        preferenceScreen.findPreference<SwitchPreferenceCompat>("HideIcon")?.setOnPreferenceChangeListener { _, newValue ->
            val component = ComponentName(MyApplication.appContext, "com.tsng.hidemyapplist.MainActivityLauncher")
            ...
            MyApplication.appContext.packageManager.setComponentEnabledSetting(component, status, PackageManager.DONT_KILL_APP)
            true
        }
    }
